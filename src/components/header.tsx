...
type NavKey = 'home' | 'about' | 'services' | 'experience' | 'projects' | 'contact';
...
export const Header = () => {
  ...
  // Only render Dialog after mount to avoid SSR/client ID mismatch
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    ...
    >
      {/* Mobile menu — only rendered client-side to avoid Radix hydration mismatch */}
      {mounted && (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button variant="outline" size="lg" className="bg-background/80 backdrop-blur-sm sm:hidden">
                ...
                {links.map(({ name, hash }) => (
                  ...
                  >
                    <Link className="block" href={hash}>
                      {tNav(name.toLowerCase() as NavKey)}
                    </Link>
                  </li>
                ))}
              </ul>
